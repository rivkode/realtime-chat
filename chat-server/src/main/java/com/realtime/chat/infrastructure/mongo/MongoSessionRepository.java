...
/**
 * chat-server 측 {@link SessionRepository} 구현(MongoDB).
 *
 * <p>chat-server는 주로 {@code findById}로 join/leave 시점 세션 존재·종료 여부를 검증한다.
 * 세션 생성/종료/목록 같은 라이프사이클 변경은 api-server 책임이라 chat-server에서는 거의 호출되지 않는
 * {@code save}/{@code findByFilter}도 인터페이스 충족을 위해 구현해 둔다 — 향후 필요해지면 그대로 사용 가능.
 */
@Repository
